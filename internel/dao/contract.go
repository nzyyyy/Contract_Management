...
func (d Dao) GetContractById(id int) (*model.ContractWithFull, error) {
	c := model.Contract{ID: id}
	return c.GetContractById(d.engine)
}
func (d Dao) GetContractWithoutOperator() ([]*model.APIContract, error) {
	return model.Contract{}.ListWithoutOperator(d.engine)
}
